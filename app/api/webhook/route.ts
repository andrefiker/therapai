// app/api/webhook/route.ts            
  ...
  // Locked decisions: D9, D10, D11, D12 (DECISIONS.md). Spec:            
  WEBHOOK_SPEC.md.                                                        
  //                                                                      
  // Flow per request:                                                    
  //   1. Verify X-Fireflies-Secret header.    
  //   2. Parse payload, extract Fireflies transcript ID.                 
  //   3. Idempotency check: if fireflies_id already exists in            
  therapai_sessions, no-op 200.                                           
  //   4. Insert session row (status='processing'); insert may race-fail  
  on the unique index, treat as no-op.                                    
  //   5. Fetch full transcript via Fireflies GraphQL.
  //   6. Identify patient (title-first; fallback to most-frequent        
  non-André speaker).                                                     
  //   7. Match against existing therapai_patients by fuzzy name; on no   
  match → status='unidentified'.                                          
  //   8. Run molar analysis (Claude → retry → OpenAI fallback → mark     
  failed).                                                                
  //   9. Save analysis row.              
  //  10. Rebuild longitudinal report for the patient (every new session, 
  per D9/D12).                                                            
  //  11. Update session status='done', set model_used, set patient_id.
  //  12. Return 200.                                                     
  //                                      
  // Errors mid-flow update the session row to 'failed' and return 200    
  (don't make Fireflies retry).                                           
  ...
  // V1 — single therapist hardcoded. Multi-tenant onboarding will derive 
  this from auth.                                                         
  const ANDRE_THERAPIST_ID = 'a0000000-0000-0000-0000-000000000001';      
                                                                          
  // Patterns to skip when matching speaker names — these are André's     
  voice across speaker labels.                                            
  const ANDRE_NAME_PATTERNS = ['andre', 'andré', 'fiker', 'ghost'];       
                                                                          
  // Title-skipping patterns (Fireflies generic titles not derived from   
  clinical content).                                                      
  const GENERIC_TITLE_PATTERNS: RegExp[] = [                              
    /^meet\b/i,                                                           
    /^(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)[a-z]*\s+\d/i,
    /^[a-z]{3,4}-[a-z]{3,4}-[a-z]{3,4}$/i, // Google Meet codes like      
  xyz-abc-def                                                             
  ];                                                                      
  ...
  ────────────────────────                     
  const MOLAR_SYSTEM_PROMPT = `Você é um clínico especialista em Análise  
  do Comportamento e RFT.                                                 
  Avaliações rigorosas, funcionalmente fundamentadas, em português 
  brasileiro técnico.                                                     
  Sem linguagem vaga. Cada afirmação ancorada em dados da transcrição.
  Diferencie dados de inferências. Cite falas com timestamp. Sem          
  preambles.                                                              
  ...
  3. Análise Funcional                                                    
     ...
     3.2 Contingências                                                    
     3.3 RFT: molduras de avaliação / identidade / temporais / causais / 
  transformação de função / coerência relacional                          
  4. Manejo Terapêutico                   
  5. Sugestões de Intervenção                                             
  6. Prognóstico                          
  7. Hipóteses para Próxima Sessão`;                                      
                                                                          
  const LONGITUDINAL_SYSTEM_PROMPT = `Você é um clínico especialista em   
  Análise do Comportamento e RFT.                                         
  Análise longitudinal rigorosa, em português brasileiro técnico,         
  sintetizando todas as sessões anteriores deste paciente.                
  Sem preambles.                               
  ...
  export async function POST(req: NextRequest): Promise<NextResponse> {   
    ...
    if (!firefliesId) {                        
      return NextResponse.json({ error: 'missing_transcript_id' }, {
  status: 400 });                                                         
    }                                    
   ...
   {                                           
    ...
    // 3. Idempotency: short-circuit if we've already processed this      
  fireflies_id to completion.
  ...
  sessionId: existing.id });                                              
    ...
    try {                                                                 
      ...
      await markSessionFailed(supabase, existing?.id, firefliesId,        
  `fireflies_fetch_failed: ${(err as Error).message}`);                   
      return NextResponse.json({ ok: true, status: 'failed', reason:      
  'fireflies_fetch' });                                                   
    }                                          
  ...
  sentencesToTranscriptText(transcript.sentences);                        
    ...
    try {
      ...
      await markSessionFailed(supabase, sessionId, firefliesId,           
  `analysis_failed: ${(err as Error).message}`);
      return NextResponse.json({ ok: true, status: 'failed', reason:      
  'analysis' });                                                          
    }                                          
                                                                          
    // 8. Save analysis row + update session row.                         
    const sessionNumber = await nextSessionNumberForPatient(supabase,
  patientId);                                                             
    ...
    // 9. Rebuild longitudinal for this patient (D9/D12 — every new       
  session).         
    try {                                                                 
      await rebuildLongitudinalForPatient(supabase, patientId);           
    } catch (err) {                            
      // Longitudinal failure is non-fatal — session analysis is already  
  saved.                                                                  
      console.error('[webhook] longitudinal rebuild failed (non-fatal)',
  err);                                                                   
    }                                     
  ...
  // ─── Fireflies GraphQL                
  ────────────────────────────────────────────────────────
  async function fetchFirefliesTranscript(id: string):
  Promise<FirefliesTranscript> {                                          
    ...
    const json = (await res.json()) as { data?: { transcript?:
  FirefliesTranscript }; errors?: unknown };                              
    if (json.errors) {                    
      throw new Error(`fireflies_graphql_error:                           
  ...
  async function matchOrNullPatient(supabase: SupabaseClient,             
  candidateName: string): Promise<string | null> {
  ...
  // Levenshtein-based similarity, normalized to [0, 1]. Cheap; fine for  
  short patient names.                         
  function similarity(a: string, b: string): number {                     
    ...
    const n = a.length;                  
    const m = b.length;                                                   
    const dp: number[][] = Array.from({ length: n + 1 }, () => new Array(m
   + 1).fill(0));                                                         
    ...
    for (let i = 1; i <= n; i++) {        
      for (let j = 1; j <= m; j++) {                                      
        ...
        dp[i][j] = Math.min(dp[i - 1][j] + 1, dp[i][j - 1] + 1, dp[i -
  1][j - 1] + cost);                                                      
      }                                   
    }                                                                     
    const dist = dp[n][m];                
  ...
  async function upsertProcessingSession(                                 
    ...
    if (error || !data) {
      ...
      throw new Error(`session_insert_failed: ${error?.message ??         
  'unknown'}`);                                                           
    }                                          
    return data.id;                                                       
  }                                       
  ...
  async function nextSessionNumberForPatient(supabase: SupabaseClient,    
  patientId: string): Promise<number> {
  ...
  async function runAnalysisWithFallback(                                 
    ...
    for (let attempt = 0; attempt < 2; attempt++) {                       
      try {
        ...
        console.error(`[webhook] claude attempt ${attempt + 1} failed`,   
  err);                                                                   
        if (attempt === 0) {                   
  ...
  Promise<string> {
    ...
    if (!block || block.type !== 'text') throw new                        
  Error('claude_no_text_block');
    return block.text;                                                    
  }                                       
  ...
  // ─── Prompt builders                                                  
  ──────────────────────────────────────────────────────────
  function buildMolarUserPrompt(t: FirefliesTranscript, transcriptText:   
  string): string {                       
    const meta = [                             
      ...
      t.summary?.overview ? `Resumo Fireflies: ${t.summary.overview}` :   
  null,                                                                   
    ].filter(Boolean).join('\n');                                         
                                                                          
    return `Transcrição da sessão clínica abaixo. Produza a análise       
  completa nas 7 seções obrigatórias.          
  ...
  function buildLongitudinalUserPrompt(patientName: string, analyses: {   
  session_number: number | null; analysis_md: string | null; }[]): string
  {                                                                       
    const sorted = [...analyses].sort((a, b) => (a.session_number ?? 0) -
  (b.session_number ?? 0));                                               
    const body = sorted
      .map((a) => `=== Sessão ${a.session_number ?? '?'}                  
  ===\n${a.analysis_md ?? ''}`)                                           
      .join('\n\n');                           
    return `Análises de sessão para ${patientName} abaixo. Sintetize a    
  análise longitudinal completa nas 8 seções obrigatórias.                
  ...
  ─────────────────────────────────────────────────────
  async function rebuildLongitudinalForPatient(supabase: SupabaseClient,
  ...
  // ─── Helpers                          
  ──────────────────────────────────────────────────────────────────
  function sentencesToTranscriptText(sentences: FirefliesSentence[]):
  string {                                                                
    return sentences                     
      .map((s) => {                                                       
