...
function MarkdownViewer({ md }: { md: string }) {
  const html = md
    ...
    .replace(/\n\n/g, '<br/><br/>')
  return <div className="text-slate-700 text-sm leading-relaxed" dangerouslySetInnerHTML={{ __html: html }} />
}
...
export default async function DemoPatientPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!patient) {
    return (
      <div className="text-center py-20">
        ...
        <Link href="/demo" className="text-indigo-600 text-sm mt-4 inline-block">← Voltar</Link>
      </div>
    )
  }
  ...
  return (
    <div>
      <div className="mb-6">
        <Link href="/demo" className="text-sm text-slate-400 hover:text-slate-600 mb-3 inline-block">
          ← Todos os pacientes (demo)
        </Link>
      ...
      <div className="grid grid-cols-3 gap-6">
        ...
        <div className="col-span-2 space-y-6">
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-xs text-amber-900">
            <strong>Demo somente-leitura.</strong> Chat de caso, confirmação de afirmações e briefings só funcionam com conta provisionada. <Link href="/#lista" className="underline">Entre na lista de espera</Link> para testar com seus pacientes.
          </div>
            ...
            return (
              <div key={s.id} className="bg-white rounded-xl border border-slate-200 p-6">
                <h2 className="text-base font-semibold text-slate-900 mb-4">
                  ...
                  {molecular && (
                    <span className="ml-2 text-xs text-indigo-600 bg-indigo-50 border border-indigo-200 px-2 py-0.5 rounded-full">+ molecular ({molecular.events_count ?? '?'} eventos)</span>
                  )}
                </h2>
                <div>
                ...
                {molecular && (
                  <div className="mt-6 pt-6 border-t border-slate-100">
                    <h3 className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-2">Molecular ({molecular.events_count ?? '?'} eventos discretos)</h3>
